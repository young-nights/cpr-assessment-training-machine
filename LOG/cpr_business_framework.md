...
详细状态机参见 [业务需求.md §1.4](./业务需求.md#14-颈动脉模拟状态机)。
...
3. **状态同步**:光栅板无需接收其他控制指令,仅负责数据采集和上报
...
> 注意：按压和吹气不同时进行，共用同一帧格式。当 `direction_blow != 0`（吹气阶段）时发送吹气帧（TYPE=0x02），其余时间发送按压帧（TYPE=0x01）。Sensor 板通过 TYPE 字段直接区分数据含义。
...
```c
// STM8 光栅板 → Sensor 板 串口数据帧 (64字节, packed)
typedef struct __attribute__((packed)) {
...
用户吹气 → 光栅编码器 → Raster板(中断计数) → UART发送潮气量
    → Sensor板(UART2接收) → ADC128S102CIMTX(薄膜压力传感器点位检测) → MPU6050(头部上仰角度)
...
│                                                                  │
│  主循环 (while(1))                                               │
│  ├─ depth_mm = depth_count * MM_PER_PULSE                       │
│  ├─ USART1_SendDepthDir(depth_mm, direction)                    │
│  └─ Delay_ms(100)                                               │
│                                                                  │
│  定时器 (TIM1)                                                   │
│  └─ 系统滴答 (Systick)                                          │
