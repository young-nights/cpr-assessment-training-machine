...
1. **实时数据上报**:每100ms通过UART1发送8字节实时数据帧（含类型标记）
2. **停止指令接收**:Sensor板通过UART1发送停止指令, 帧格式 `0xAA + 0x02 + 0x03 + 0xFF + CHK + 0x55`（6字节）
3. **成绩数据上报**:收到停止指令后,计算最终成绩,发送70字节成绩数据帧
4. **状态同步**:光栅板无需接收其他控制指令,仅负责数据采集和上报

#### 成绩计算职责边界

详细的责任划分参见 [业务需求.md §3.3](./业务需求.md#33-成绩数据计算职责边界)。

**在光栅板计算**:按压深度、频率、正确率、回弹判定、吹气参数、循环统计、最终结果
**传递到 Sensor 板计算**:按压位置(需 MPU6050)、头部上仰(需 MPU6050)、震动检测(需压电 ADC)、异物检测(需 CC6201)
...
| **5 Raster → Sensor** | Raster 板 | Sensor 板 | UART (115200) | 串口直连 | 光栅实时深度+方向数据 |
| **6 Sensor ⇄ Mainboard** | 双向 | 双向 | RS485 Modbus | 有线 | 无线故障时的有线备用通道 |
| **7 Raster → Sensor (成绩)** | Raster 板 | Sensor 板 | UART (115200) | 串口直连 | STM8 成绩数据上报 |
| **8 Sensor → Mainboard (成绩)** | Sensor 板 | Mainboard 板 | nRF24L01 | Pipe1 | 成绩数据转发 |
...
**用途**:光栅板 → Sensor 板,每 100ms 发送一次当前按压/吹气深度和方向
...
- **帧头 (0xAA)**:固定标识
- **长度 (0x04)**:数据域长度(DH+DL+TYPE+DIR = 4字节)
- **深度**:2 字节,单位 0.1mm(`depth_mm * 10`)
...
**物理参数**:UART1, 115200bps, 8N1
**用途**:光栅板 → Sensor 板,停止指令触发后发送完整成绩数据

#### 5.3.1 成绩数据结构

STM8 光栅板作为成绩计算的核心,通过 UART 串口将完整成绩单数据上报给 Sensor 板:
...
**字段说明**:
- **帧头 (0xAA)**:固定标识,与实时数据帧相同
- **长度 (LEN)**:从命令字节到 CRC 前的数据长度,固定为 0x43 (67)
- **命令 (0x02)**:表示成绩数据上报(区别于 0x01=开始、0xFF=停止)
- **数据 (RasterReport_t)**:64 字节成绩数据
- **CRC**:CRC16-Modbus 校验
- **帧尾 (0x55)**:固定标识

**与实时帧的区分**:
- 实时帧长度 = 0x04,成绩帧长度 = 0x43,通过长度字段即可区分
- 命令字节进一步明确帧类型

**触发条件**:
- 训练/考核/竞赛模式结束时,Mainboard 发送停止指令
- Sensor 板转发停止指令到光栅板
- 光栅板收到停止指令后,计算最终成绩并上报

**数据流向**:
```
STM8 光栅板 (成绩计算完成)
    │ UART1 发送成绩帧 (70 Bytes)
    ▼
Sensor 板 (UART2 接收)
    │ nRF24L01 CMD_RASTER_REPORT (0x70) 转发
    ▼
Mainboard 板 (接收并存储)
    │ Flash 写入
    ▼
