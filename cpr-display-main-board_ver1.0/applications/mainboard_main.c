...
extern int LightBar_Thread_Init(void);
extern int NixieTube_Thread_Init(void);
extern int Printer_Thread_Init(void);
extern int Touch_Thread_Init(void);
extern int WT588D_Thread_Init(void);
extern int start_thread_Init(void);
...
int main(void)
{
  ...
  // 初始化所有线程（统一在 main 中启动）
  nRF24L01_Thread_Init();
  LightBar_Thread_Init();
